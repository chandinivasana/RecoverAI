...
| 7 | Backend/deploy hygiene (CORS, env, Docker build args) | ✅ Done | CORS env-driven explicit origins (no more `*`+credentials); postgres password via `.env`; `NEXT_PUBLIC_API_URL` as a Docker BUILD arg (was dead runtime env pointing at an unresolvable host); frontend image now copies `.npmrc` (npm ci previously failed on Blade's peer chain); per-context `.dockerignore` (backend `venv/` no longer shipped); backend healthcheck + health-gated frontend startup; SPEC phantom `/execute` removed; deployment agent-context rewritten. Verified: full `docker compose up` — 4 containers healthy on **Postgres**, seeded+warm-started, chain intact (309 events), browser loads live KPIs from the containerized stack. 63/63 tests. |
| 8 | Mermaid diagrams + docs rewrite (honest claims everywhere) | ✅ Done | Five Mermaid diagrams (hero pipeline + policy-gate ladder in README; container topology, executor-truthful payment state machine, replay pipeline in ARCHITECTURE.md) — all 5 validated against the real Mermaid 11 parser. README fully rewritten: honest held-out numbers (₹6.24L/5.04%, Brier 0.2168, ECE 0.0979, FP cost ₹4.61L, cost-of-safety ₹3.05L), LLM-layer and audit-chain sections, accurate red-team table with earned verdicts, hard-vs-escalation-class rule semantics, 63-test breakdown, agent-aware repo section, accurate quickstart with LLM env flags. Stale-claim sweep clean (no 8.76%/0.0811/24-24/cryptographic/Next-14 anywhere). |
| 9 | CI upgrade + auto-refreshing `.agents/context/` on merge to main | ✅ Done | `scripts/generate_context.py` (deterministic, idempotent) generates repo-map / api-surface (all 29 routes introspected from the live app) / test-inventory (70 tests) / db-schema into `.agents/context/`. `context-refresh.yml` bot-commits refreshed context on every push to main (double loop-guard). `ci.yml`: ruff (backend ruff-clean after a 319-finding sweep, config in `pyproject.toml`), 70 deterministic tests incl. new `tests/test_api_integration.py` (7 HTTP-contract tests: demo recovery, ₹2.5L refusal→review, eval byte-determinism, human-approval 409 on injection, earned red-team verdicts, chain verify, tenant filtering), eslint, Next build as the TS gate, docker image builds with GHA cache, PR-only context-freshness check. AGENTS.md rewritten to best practice; CLAUDE.md = `@AGENTS.md` + deltas. |

**🎉 All 10 phases complete (Aug 23, 2026).** Remaining follow-ups are listed below (GitHub remote + real CI badge, AWS deploy, pitch video).
